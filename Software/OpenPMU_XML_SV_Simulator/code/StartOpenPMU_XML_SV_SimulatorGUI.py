...
class MainWindow(BaseTemplate):
    def __init__(self):
        ...
        self.dataThread = PMUCape(
            self.frequencies,
            np.array([0, 1, 2, 0, 1, 2]) * np.pi * 2 / 3,
            self.channels,
        )
    ...
    def writeSettings(self):
        settings = QSettings("grass", "PMUCapeSimulatorGUI")
    ...
    def readSettings(self):
        ...
        for s in spins:
            s.setValue(int(settings.value(s.objectName(), 50)))

        dials = self.findChildren(QDial)
        for d in dials:
            d.setValue(int(settings.value(d.objectName(), 50)))
