...
MAX_HARMONICS = 50  # DIN EN 50160 considers up to 50th harmonic
...
def WindowedDFT(samples, Fs, Freq, harmonic_order=1):
    N = len(samples)
...
def CalculateHarmonicsWDFT(samples, Fs, Freq, max_harmonic=MAX_HARMONICS):
    harmonics = []
    for h in range(1, max_harmonic + 1):
        mag, _ = WindowedDFT(samples, Fs, Freq, harmonic_order=h)
...
def CalculateNoiseWDFT(samples, Fs, Freq, max_harmonic=MAX_HARMONICS):
    N = len(samples)
    reconstructed = zeros(N)
    for h in range(1, max_harmonic + 1):
        mag, phase = WindowedDFT(samples, Fs, Freq, harmonic_order=h)
...
def Algorithm_CSV(Freq, Fs, NumHarmonics, FileName):
    ...
    for x in range(0, CompCycles):
        ...
        fundamental, harmonics, thd = CalculateHarmonicsWDFT(
            CompWindowBuffer, Fs, current_freq, max_harmonic
        )
        Theta_Rad = WindowedDFT(CompWindowBuffer, Fs, current_freq, harmonic_order=1)[1]
        noise_rms = CalculateNoiseWDFT(CompWindowBuffer, Fs, current_freq, max_harmonic)
        if x == 0:
...
def Algorithm_UDP(UDPPort, NumHarmonics):
    ...
    for x in range(3000):
        ...
        Theta_Rad = WindowedDFT(CompWindowBuffer, Fs, Freq, harmonic_order=1)[1]
        noise_rms = CalculateNoiseWDFT(CompWindowBuffer, Fs, Freq, max_harmonic)
        if x == 0:
            EstimatedFreq = Freq
            Prev_Theta_Rad = Theta_Rad
        else:
